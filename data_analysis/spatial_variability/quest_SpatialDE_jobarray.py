...
# st_dir = "/share/fsmresfiles/SpatialT/" # On FSM servers
st_dir = "/projects/b1131/SpatialT/" # On Quest
all_samples = pd.read_csv(st_dir + "master_table_new.txt", sep = "\t") # 1735 samples

sample_dir = sys.argv[1]
print(">>> " + sample_dir + " started <<<")

### Read counts and coordinates
counts = pd.read_csv(sample_dir + 'processed/counts.csv')
...
results.to_csv(sample_dir + 'analysis/SVG/SpatialDE_results.tsv', sep = '\t', index = False)
...
print("# Whole tissue SVG analysis finished.")
...
if sign_results.shape[0]>0:
    ...
    count = pd.DataFrame(sign_results['l'].value_counts())['l'].tolist()
    ...
    histology_results, patterns = SpatialDE.aeh.spatial_patterns(X, resid_expr, sign_results, C = n_patterns, l = L, verbosity = 1, delta_elbo_threshold = 1)
    print("# Pattern analysis finished.")
else:
    patterns = pd.DataFrame(columns=['0', '1'])
    histology_results = pd.DataFrame(columns=['g', 'pattern', 'membership'])
    print("# [WARNING] Cannot perform pattern analysis, no sig genes.")

### Write results to file
if not os.path.exists(f'{sample_dir}analysis/SVG'):
    os.makedirs(f'{sample_dir}analysis/SVG')
histology_results.to_csv(sample_dir + 'analysis/SVG/SpatialDE_histology_results.tsv', sep = '\t', index = False)
patterns.to_csv(sample_dir + 'analysis/SVG/SpatialDE_patterns.tsv', sep = '\t', index = False)
